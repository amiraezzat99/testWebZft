...
export const createCoupon = async (req, res, next) => {
  ...
  if (amount > 100) {
    return next(
      new Error('amount not valid (must be from 1 to 100)', { cause: 400 }),
    )
  }
  ...
  if (moment(toDateMoment).isBefore(moment(fromDateMoment))) {
    return next(
      new Error('please enter dates start from tomorrow', { cause: 400 }),
    )
  }
  ...
  return res
    .status(201)
    .json({ message: `Coupon Code ${code} added successfully` })
}
...
export const updateCoupon = async (req, res, next) => {
  ...
  if (req.body.fromDate) {
    ...
    if (moment(new Date(req.body.fromDate)).isAfter(moment(coupon.toDate))) {
      return next(
        new Error('coupon cannot start after the expiration date', {
          cause: 400,
        }),
      )
    }
    coupon.fromDate = moment(new Date(req.body.fromDate)).format(
      'YYYY-MM-DD HH:mm',
    )
  }
  ...
  if (req.body.toDate) {
    ...
    if (moment(new Date(req.body.toDate)).isBefore(moment(coupon.fromDate))) {
      return next(
        new Error('coupon cannot expired before the starting date', {
          cause: 400,
        }),
      )
    }
    if (moment(new Date(req.body.toDate)).isSame(moment(coupon.fromDate))) {
      return next(
        new Error('coupon cannot start on same day of expiration', {
          cause: 400,
        }),
      )
    }
